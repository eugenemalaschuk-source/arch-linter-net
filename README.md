...
The exact JSON route remains available for integrations that need the canonical
byte string. Project canonical Architecture Health and canonical policy
inventory into that snapshot-compatible Shields payload without rerunning
analysis:
...
An origin guarantee cannot recall copies already held by a browser, GitHub's
README image proxy (Camo), another proxy, or an offline client. If a cached
image looks old, compare the origin representation and headers with the
observed copy; cache delay is not current-origin proof, and no fixed delay is
promised. Publication never creates cache-busting commits or mutates canonical
payload values merely to make a refresh visible. See
