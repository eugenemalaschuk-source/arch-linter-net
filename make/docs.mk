...
docs-build:  ## Build static documentation site
	@cd "$(PROJECT_ROOT)" && UV_PROJECT_ENVIRONMENT="$(PROJECT_ROOT)/.venv" "$(UV)" run --project tools/pyproject.toml python3 tools/scripts/filter_mkdocs_warnings.py -- mkdocs build
...
lint-docs:  ## Verify MkDocs documentation structure and lint markdown
	@cd "$(PROJECT_ROOT)" && UV_PROJECT_ENVIRONMENT="$(PROJECT_ROOT)/.venv" "$(UV)" run --project tools/pyproject.toml python3 tools/scripts/filter_mkdocs_warnings.py -- mkdocs build --strict
