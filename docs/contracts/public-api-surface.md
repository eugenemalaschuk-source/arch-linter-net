...
class Acme.Ledger.Module.LedgerEntry
ctor Acme.Ledger.Module.LedgerEntry(System.Decimal)
property Acme.Ledger.Module.LedgerEntry.Amount: System.Decimal
```
...
Removals and changes are reported for the union of the contract's declared surface. Inline `declared_api` entries carry no assembly attribution, so a removal that came from an inline list is reported without an assembly name; snapshot entries carry theirs.
