...
35 production `static class` declarations existed under `src/` (none in `src/ArchLinterNet.Unity/`) before #158's change converted `ArchitectureContractExecutor`, leaving 34 classified below. #159 converted the remaining 12 production service/orchestrator/scanner/resolver/parser/loader classes tracked in section (e) below (2 of which held hidden `static Lazy<T>` global state); the 20 pure-helper/extension/facade classes in sections (a), (b), and (d) are unchanged and remain intentionally static.
...
These owned real logic; all are now instance classes. #158 converted the one the issue named explicitly (`ArchitectureContractExecutor`); #159 converted the remaining 12, none of which remain a `static class` declaration.
...
| `Core/Discovery/ArchitectureProjectDiscovery.cs` | *(deleted)* | **Converted** (#159) | Logic moved into `ArchitectureProjectDiscoveryService` (`IArchitectureProjectDiscoveryService`, registered `AddSingleton`), which now constructor-injects the two parsers below directly |
...
| `Core/Contracts/ArchitectureBaselineLoader.cs` | *(deleted)* | **Converted** (#159) | Folded into `ArchitectureBaselineLoadingService` (`IArchitectureBaselineLoadingService`, registered `AddSingleton`) as `internal static` methods reachable only through the composed service class — not a standalone `static class`, and its only production caller |
...
| `Core/Discovery/ArchitectureSolutionParser.cs` | Converted (#159) | `IArchitectureSolutionParser` / `internal sealed class`; a private constructor-injected collaborator of `ArchitectureProjectDiscoveryService`, not separately registered in `AddArchLinterNetCore()` (single caller) |
| `Core/Discovery/ArchitectureProjectFileParser.cs` | Converted (#159) | `IArchitectureProjectFileParser` / `internal sealed class`; same single-caller treatment as the solution parser above |
