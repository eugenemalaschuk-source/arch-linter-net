...
| `layout-conventions` | `Abstractions` folders contain only interfaces or abstract classes; `Exceptions` folders contain only exception-role declarations; every handwritten production type has one source declaration | Source declaration kind, `abstract` modifier, exception classification, and the path-complete source declaration inventory | **adopt (strict)** | Folder-purity rules and `production-types-have-one-source-declaration` are strict controls over `src`. The declaration-count rule uses `max_declarations_per_type: 1` with no reviewed aggregate waivers or second audit authority; all known production aggregates were resolved before adoption. Test fixtures, generated declarations, and intentional partial-language/interop samples remain outside the production selector and retain dedicated source-index coverage. |
