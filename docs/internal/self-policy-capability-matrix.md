...
| `layout-conventions` | `Abstractions` folders contain only interfaces or abstract classes; `Exceptions` folders contain only exception-role declarations; production partial-type aggregates cannot grow past a reviewed baseline | Source declaration kind, `abstract` modifier, exception classification, and source declaration count | **adopt (strict)** | Focused folder-purity rules protect the migrated convention without imposing filename or partial-type conventions. `production-partial-type-declaration-count-does-not-increase` (#742) reuses the audit rule's `max_declarations_per_type: 1` evidence as a strict ratchet: today's 19 known aggregates are frozen as exact-match `ignored_violations` reviewed debt, so a new or grown aggregate fails the gate while `decompose-god-classes` remains the authority for removing the debt entirely. Remaining declaration-count and role-placement debt stays separately in audit via `production-types-have-one-source-declaration`, unchanged. |
