...
Every sample is one real `dotnet <ArchLinterNet.Cli.dll> --policy ... --profile <path>` process invocation against an isolated copy of the fixture. `AnalysisMs` is derived from the profile's own `Phases`: the single-mode path's explicit `total` phase, or — for the combined `--mode strict,audit` path, which has no `total` wrapper (`ArchitectureValidationApplicationService.CreateSnapshot` doesn't wrap in `Measure("total")` the way single-mode `Validate` does) — the sum of that profile's top-level (`Indent: 0`) phases. `PreflightMs` is the `build_state_preflight` phase alone, isolating MSBuild-driven `--ensure-built` verification from the rest of ArchLinterNet's own analysis work, per the issue's "separate restore/build/preparation time from ArchLinterNet analysis time" requirement. Median/p95 are computed over 9–10 samples per scenario (scenario 1 is inherently `n=1`, the single first cold run).

## Results

| Scenario | n | Median analysis (ms) | p95 analysis (ms) | Median preflight (ms) | Completion status |
|---|---|---|---|---|---|
| 1 — cold process, warm filesystem, strict | 1 | 348 | 348 | 3806 | Success |
| 2 — immediate warm strict repeat (no persistent cache — #365) | 9 | 347 | 358 | 3216 | Success |
| 3 — strict + audit as separate legacy-style processes (paired sum) | 10 | 688.5 | 700 | 3169¹ | Success/Success |
| 4 — combined strict+audit from one #363 snapshot | 10 | 327.5 | 336 | 3142.5 | Success |
| 5a — one report sink (`--report json=stdout`) | 10 | 342 | 360 | 3193 | Success |
| 5b — three report sinks (`--report human/json/sarif`, one analysis) | 10 | 342.5 | 347 | 3164.5 | Success |
| 7b — validation-failure completion path | 10 | 369.5 | 377 | 3175 | ValidationFailure |
| 7c — preparation-failure completion path (never built, `--no-restore`, no receipts) | 10 | 285.5 | 297 | 4 | PreparationFailure |
...
- **The #363 one-snapshot benefit is visible end to end**: scenario 3 (two separate processes, ~688.5ms combined analysis time) versus scenario 4 (one process, one snapshot, ~327.5ms) shows roughly half the analysis time when both modes are served from one composed snapshot instead of two independent runs — consistent with #363's own invariant tests, now demonstrated through the CLI's real machine-readable output.
- **`--ensure-built` preflight dominates wall-clock time on this fixture** (~3.1–3.8s per run) regardless of cold/warm state, because `--ensure-built` shells out to `dotnet build` for verification on every invocation against ten projects; scenario 1 vs. 2's difference (~590ms) is real but small relative to this fixed MSBuild verification cost. This is exactly the kind of cost #365's persistent cache is expected to address, and is why this evidence separates `PreflightMs` from `AnalysisMs` rather than reporting one blended number.
- **Multi-sink output adds no measurable analysis cost** (scenario 5a ~342ms vs. 5b ~342.5ms), confirming `multi-sink-output`'s "one analysis serves all sinks" invariant holds in a real end-to-end run, not just in the unit-level `AnalysisProfileBuilderTests` coverage.
