...
There is a second, earlier pipeline with the same shape: `IArchitecturePolicyRawDocumentValidator` instances (`ArchitecturePolicyRawDocumentValidatorPipeline.All`, `src/ArchLinterNet.Core/Contracts/RawValidators/`) run against the effective YAML node tree *before* deserialization, because `IgnoreUnmatchedProperties()` erases the unknown keys they exist to reject. The two pipelines are deliberately separate — different inputs, different points in the load — and neither owns the other's ordering. The loader itself references no YamlDotNet representation-model type at all — not in a signature, not inside a method body. `PolicyRawValidationSeamTests` enforces that against both the loader's source and its compiled members, locals and captured state, because the checks this replaced took only a YAML string and built the node tree internally.
