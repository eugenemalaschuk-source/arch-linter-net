...
| `crypto_keylength` | **Met (unchanged, evidence corrected — was previously missing the real product mechanism)** | `AnalysisCacheHmacKeyStore` generates a 32-byte (256-bit) HMAC key via `RandomNumberGenerator.GetBytes(KeyLengthBytes)` (`KeyLengthBytes = 32`), used with HMAC-SHA256 — well above adequate key-length guidance for a MAC key. This is the project's actual security mechanism; the prior evidence cited only the release pipeline's SHA-256/Sigstore key material and missed it. |
