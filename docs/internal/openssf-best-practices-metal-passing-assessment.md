...
| `release_notes_vulns` | N/A (unchanged; N/A is an accepted value for this criterion, evidence broadened) | No publicly known runtime vulnerability has been fixed in any release, so there is nothing to identify. `gh api repos/eugenemalaschuk-source/arch-linter-net/security-advisories` returning `[]` only proves no repo-scoped GitHub Security Advisory exists; broadened with the same public-database check used for `vulnerabilities_fixed_60_days` below: dated 2026-08-23, `POST https://api.osv.dev/v1/query` for `ArchLinterNet.Cli` and `ArchLinterNet.Core` (NuGet) each return `{}` — no publicly known CVE/OSV runtime vulnerability against either published package to have identified in any release's notes. |
...
| `test_most` | Met (unchanged, evidence corrected — was citing the wrong metric) | This criterion asks whether the test suite covers most of the code's branches/inputs/functionality, which branch coverage measures directly; branch protection's `code_coverage` rule (`minimum_coverage: 80`) tracks *line* coverage, a different, weaker proxy for "most branches." Reuses the same measurement as the `dynamic_analysis` row below: merged branch coverage across this PR head's four CI shards is 81.6% (15,069/18,449) as of 2026-08-23 — most of the codebase's branches are exercised by the automated test suite. |
...
| `delivery_unsigned` | **Met (changed from Unmet)** | The literal criterion only prohibits retrieving a cryptographic hash over unencrypted HTTP and trusting it without a signature check. NuGet.org, GitHub Releases, and GitHub Pages have always been HTTPS-only for this project (`grep -rn 'http://' docs/ README.md SECURITY.md .github/workflows/*.yml` finds no plain-HTTP link), so a hash has never been fetched over an unauthenticated channel — that alone already satisfies the criterion as written and was true before this task. The #611/#612/#627 release-provenance work (merged August 2026) is a materially *stronger* guarantee on top — every release `.nupkg`/`.snupkg` and the canonical manifest/checksum files now carry GitHub build provenance attestation, verified with `gh attestation verify` before any digest is trusted — but it is not the reason the literal criterion passes; it is additional evidence beyond what the criterion strictly requires. The July `Unmet` answer looks like a conservative/inherited answer rather than a genuine repository gap this re-audit needed to close. Evidence: [Release provenance verification guide](../guides/release-provenance-verification.md), `.github/workflows/release-nuget.yml`. |
