...
| Registry and schemas | Immutable release-qualified package registry | The manifest lists policy root/fragment, baseline v2, API snapshot, normalized finding, build state, cache, and profile. Resources retain their own immutable 0.5.1 or independently advanced 0.6.1 IDs and digests; profile correctly declares write-only support. |
| Finding and baseline identity | `finding/v2` (legacy v1 readable); baseline v2 with `identity_version: 1` | Documentation and compatibility spec keep identity separate from display text, paths, timings, and report destinations; migration guidance preserves v1 reader semantics and explicit requalification. |
