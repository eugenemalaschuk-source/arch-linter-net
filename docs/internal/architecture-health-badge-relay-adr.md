...
| Protected JOSE header `kid` | Select only a key identified by `kid` from the fixed GitHub issuer JWKS endpoint. An unknown key can trigger one coalesced refresh only when the fixed protection window permits it; concurrent misses share that refresh, unresolved keys are rejected, and a later miss after the window can discover provider rotation. Configuration stores neither an allowed key-ID set nor current/next key rotation state, and refresh bookkeeping never grows with arbitrary `kid` values. No JWT header or claim can select a network URL. Provider key rotation is controlled through the fixed issuer trust chain, not a permanent snapshot of one key |
