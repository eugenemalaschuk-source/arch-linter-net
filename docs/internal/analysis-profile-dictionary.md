...
| `Concurrency` | Issue #408's bounded parallel scanning. `Status` is `NotApplicable` (all numeric fields `0`) unless at least one scanning phase (type loading, source-file fact-index materialization) actually took the bounded-parallel code path for this run, in which case `Status` is `Active`. `MaxParallelism` is the resolved effective degree (`--max-parallelism`/`WithMaxParallelism()`, defaulting to `max(1, min(Environment.ProcessorCount, 4))`) regardless of whether any phase went parallel. `ScheduledWorkItems`/`CompletedWorkItems` count partition units (one per target assembly for type loading; one per assembly or source root for fact-index materialization). `ObservedMaxConcurrency` is the highest number of partition workers observed running concurrently. `MergeOperations` counts deterministic merge steps (one per phase that ran in parallel). | `AnalysisProfileConcurrencyCounters` |
