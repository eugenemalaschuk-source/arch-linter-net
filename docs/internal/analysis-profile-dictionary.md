...
| `Cache` | Issue #365's persistent `analysis-cache/v1`. `Status` is `NotApplicable` (all fields `0`, `Mode` `"disabled"`) unless a run configured `--cache`/`WithCache()` with anything other than disabled, in which case `Status` is `Active`. `Lookups`/`Hits`/`Misses` are populated once a genuine pre-run reuse check exists (not yet wired into the CLI/Testing execution path — see `openspec/changes/archive/*-add-analysis-cache-v1/design.md`). `Rejects`, `Writes`, `BytesRead`/`BytesWritten`, `IneligibleUnitCount`, `CorruptionEvents`, and `CancelledBeforePublish` reflect the real population attempt made after each completed, non-cancelled run. `Mode` is `"disabled"`/`"auto"`/`"path"` (never the resolved absolute cache location). `RejectReasonCounts` maps `AnalysisCacheRejectReason` name → count for this run. | `AnalysisProfileCacheCounters` |
