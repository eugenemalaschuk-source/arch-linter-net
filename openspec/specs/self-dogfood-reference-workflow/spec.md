...
The repository SHALL retain a public-safe evidence record and canonical example
...
policy/configuration identity, canonical artifact digest, and any .NET
enrichment status. The recorded forensics command SHALL use separate `--from`
and `--to` operands and SHALL NOT represent a Git revision expression as a
supported operand.

#### Scenario: Maintainer reproduces the recorded run
- **WHEN** a maintainer follows the evidence record with the named tool,
  repository revision, policy, and authored operands
- **THEN** they can recreate the canonical report and compare its digest without
  relying on a local-machine path or private adopter data
