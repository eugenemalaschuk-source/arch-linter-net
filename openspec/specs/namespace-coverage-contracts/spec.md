# namespace-coverage-contracts Specification

## Purpose
Detect first-party namespaces under configured roots that are not covered by any declared layer, namespace glob, expanded layer template, or explicit exclusion, so architecture blind spots become visible in strict and audit validation modes.
## Requirements
