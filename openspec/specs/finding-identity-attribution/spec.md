# finding-identity-attribution Specification

## Purpose
Defines who owns canonical finding-identity attribution and what it guarantees: the mapping from a
run's finding-identity candidate log to the identities attached to a contract's reported violations
lives in a dedicated component that is a pure function of (candidate log, cursor, violations), so it
is exercisable without a mutable analysis session; and its candidate ordering, per-reference
selection, tie-breaking and cancellation behavior are fixed rather than incidental to whatever the
