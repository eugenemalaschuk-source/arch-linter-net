...
When timing is enabled, the selector phase's `ElapsedMs` and `ProcessorTimeMs` SHALL be
high-resolution aggregate measurements for those invocations; when timing is unavailable, both
fields SHALL be `null`, never synthetic zero values.
