...
The system SHALL detect external dependency leakage through referenced type metadata available from project types through the current reference scanner path, including supported base types, interfaces, fields, properties, method parameters, return types, and generic arguments.
...
#### Scenario: Method-body-only usage is not guaranteed
- **WHEN** a source type only uses a forbidden external dependency inside method bodies and that usage is not visible through the current reference scanner path
- **THEN** the external dependency contract SHALL NOT be required to report that usage
