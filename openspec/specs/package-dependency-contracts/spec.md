# package-dependency-contracts Specification

## Purpose
Evaluates strict and audit package dependency contracts that forbid a named source project/assembly from declaring a `PackageReference` matching one or more forbidden NuGet package groups, complementing type-level `external_dependencies` contracts with a declared-package-reference check that does not depend on compiled code touching the package.
## Requirements
