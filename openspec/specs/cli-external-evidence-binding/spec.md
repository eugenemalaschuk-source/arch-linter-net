# cli-external-evidence-binding Specification

## Purpose
Define the packed `arch-linter-net` CLI's option surface for binding repository-local SARIF
artifacts to policy-declared `external_evidence` requirements, so the complete external-evidence
trust/selection/normalization/applicability chain delivered at the Core boundary is reachable by
local and CI invocations without custom .NET host code. This capability governs only the CLI-level
binding, merge, and exit-code integration; the underlying trust/selection/normalization/applicability
semantics remain owned by the `external-sarif-evidence`, `external-diagnostic-filtering`,
`imported-diagnostic-normalization`, and `governance-applicability-evidence` capabilities.
