# bounded-parallel-scanning Specification

## Purpose
Bound and make configurable the parallelism used for type loading (`ArchitectureTypeIndex.LoadAllTypes`) and source-file fact-index materialization (`ArchitectureSourceFileFactIndex.BuildData`) as a measured optimization, while guaranteeing byte-identical output, ordering, finding identity, cancellation, and cache-lookup semantics to the sequential path this capability replaces — deliberately excluding contract-family execution and finding-identity assignment, which remain sequential because they depend on a shared, order-sensitive candidate list.
## Requirements
