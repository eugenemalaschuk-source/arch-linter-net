...
## Purpose
Keep `ArchLinterNet.Cli`'s `Program.cs` limited to argument-based dispatch, with each CLI command's implementation owned by a dedicated class under `ArchLinterNet.Cli.Commands`, so the CLI does not regress into a monolithic god-file as commands are added.
## Requirements
