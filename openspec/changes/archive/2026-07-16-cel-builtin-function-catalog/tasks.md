...
- [x] 3.3 Run `dotnet test tests/ArchLinterNet.CEL.Tests --no-restore` and fix failures.
...
- [x] 5.1 Run `make fmt` and inspect formatting changes.
- [x] 5.2 Run `make acceptance` (lint + full test suite) and fix any issue-related failures.
...
- [x] 6.4 Re-run `dotnet test tests/ArchLinterNet.CEL.Tests --no-restore`, `make fmt`, `make acceptance`, and `openspec validate --all --strict`.
...
- [x] 7.3 Re-run `dotnet test tests/ArchLinterNet.CEL.Tests --no-restore`, `make fmt`, `make acceptance`, and `openspec validate --all --strict`.
...
- [x] 8.3 Re-run `dotnet test tests/ArchLinterNet.CEL.Tests --no-restore`, `make fmt`, `make acceptance`, and `openspec validate --all --strict`.

## 9. Review follow-up (PR #339, round 4)

- [x] 9.1 Fix `containsKey`'s `ComputeCost` again: round 3's `keyLength + entryCount` still undercounted — each of up to `entryCount` hash-collision comparisons is a `string.Equals` call that can itself scan the full key length (a near-match colliding key fails late), so real worst-case work is `keyLength * entryCount`, not `keyLength + entryCount`. New formula: fixed floor + `keyLength * (entryCount + 1)` (the `+1` folds in the guaranteed initial hash pass).
- [x] 9.2 Update `ComputeCost_ContainsKey_MatchesExactFormula` to the new formula; add `ComputeCost_ContainsKey_ChargesPerComparisonKeyLengthNotJustComparisonCount` — an adversarial regression test proving the multiplicative cost is at least an order of magnitude above what the additive (round 3) formula would have charged.
- [x] 9.3 Re-run `dotnet test tests/ArchLinterNet.CEL.Tests --no-restore`, `make fmt`, `make acceptance`, and `openspec validate --all --strict`.
