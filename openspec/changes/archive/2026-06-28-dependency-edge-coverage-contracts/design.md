...
### Decision: Multiple edges per pair are aggregated, not enumerated 1:1 in the summary
`BuildDependencyEdgeCoverageSummary` counts one `covered`/`uncovered` outcome per declared pair (mirroring how namespace coverage counts one outcome per namespace), but `CheckDependencyEdgeCoverageContract` emits one violation per *uncovered observed edge instance* (source/target namespace pair), not one per declared pair — so a policy author sees every concrete uncovered edge, not just "pair X is uncovered." This mirrors namespace coverage's per-namespace violation granularity while keeping the summary pair-oriented (matching how `between` is declared).
