...
**Non-Goals:**
- Converting all 15 identified production-service statics in this PR. The issue itself allows per-module PR splitting and explicitly warns against "rewriting all Core modules in one unreviewable PR." The other 14 (loaders with global `Lazy` state, scanners, parsers, resolvers) are documented as known follow-up work, not implemented here.
...
- **`IArchitectureContractExecutor` becomes `public`; `ArchitectureContractExecutor` stays `internal`.** `IArchitectureContractExecutor` is a constructor parameter of `ArchitectureValidationApplicationService`/`ArchitectureBaselineApplicationService`, both `public` classes, so C# requires the interface to be at least as accessible as those constructors (CS0051). The result type is a standalone top-level `public sealed record ArchitectureContractExecutionResult` (not nested inside `ArchitectureContractExecutor`), so the public interface's return type doesn't force the concrete implementation class public too — the implementation itself stays `internal sealed class ArchitectureContractExecutor : IArchitectureContractExecutor`, preserving its original encapsulation and matching the existing `ArchLinterNet.Core.Tests`/`ArchLinterNet.Unity`/`ArchLinterNet.Cli` `InternalsVisibleTo` friend-assembly pattern the test project already relies on for other internal types.
