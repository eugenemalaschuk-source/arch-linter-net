...
- **D6 hardened to fail-closed (was a silent-weakening risk, CWE-693-shaped):** a `when` at `forbidden[*]`/`allowed[*]`/`exclude[*]` that reads `dependency.*` previously compiled successfully and then always evaluated against the same fixed constants regardless of the real edge — a predicate that looks meaningful but silently never behaves as intended. `ExpressionCompilationValidator` now rejects any `dependency.` member reference in those `when` locations at policy-load time with an actionable error, converting the failure mode from silent to fail-closed. (ArchLinterNet.CEL exposes no public API to introspect a compiled predicate's referenced identifiers — confirmed by reading the full public surface of `CelCompiledPredicate`/`CelCompiledExpression`/`CelCompilationResult<T>` — so this is a source-text regex check, not a semantic AST check; scoped to `\bdependency\.` since a bare `dependency` reference can never type-check to `Bool` on its own, so every reachable use is a member access.) Two existing #163 tests that asserted `dependency.*` *compiles* were updated to assert rejection instead, plus new tests proving the rejection message and location.
- **Contextual consumer identity now includes `when`:** `RegisterContextualConsumerCore`'s `TryAdd` keyed only on role+metadata, so two selectors sharing one role/metadata shape but declaring different `when` expressions collided — the second one's `when` silently never participated in stale-selector coverage detection, order-dependent on declaration order. Identity now folds in `when` (and the paired source selector's `when`) so distinct predicates always produce distinct records.
...
- **Evaluation errors now respect `--format json`/`--format sarif`:** these errors surface deep inside contract checking (well after policy load succeeds) as a bare `InvalidOperationException` with no policy-location provenance, so they never matched `ValidateCommandHandler`'s typed-diagnostic catch and fell through to a plain-text-only generic handler regardless of requested format. The generic catch now emits a structured `architecture_execution_error` (JSON) / `architecture-execution` (SARIF) result instead of an unstructured stderr line, so CI tooling parsing `--format json`/`sarif` output gets valid output instead of a parse failure. Full policy-location enrichment for execution-time errors (matching what `EnrichValidationException` already does for load-time errors on imported policies) remains unaddressed — that requires threading `ArchitectureContractDocument`/provenance through the matcher call chain, which is a larger, separately-scoped follow-up.
