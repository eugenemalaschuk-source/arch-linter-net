...
- Extend `ArchitectureValidationBuilder` (Testing) with fluent methods mirroring `ValidationRequest`: `WithContracts(...)` (contract-ID selection), `WithBaseline(string baselinePath)` (baseline merge into the validation run), `WithUnmatchedIgnoredViolationsPolicy(bool enforce = true)`, and `WithTimings()`. `WithConditionSet`, `ValidateStrict()`, and `ValidateAudit()` keep their existing signatures and defaults.
