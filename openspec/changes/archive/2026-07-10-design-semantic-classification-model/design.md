...
| **uncovered semantic fact** | A type that received a role/metadata assignment from some source but is not matched by any `layers.<name>.selector` and is not named by an `exclusion` — the classification equivalent of coverage's `uncovered` (a fact the policy never consumes). An `override`-assigned role does **not** by itself exempt a type from this status: `override` is a classification *source* (the highest-precedence one, per `yaml_override`'s position in Decision 2), not a coverage mechanism — a type whose role came from an override is still `uncovered` unless some `selector` also matches it or an `exclusion` also names it. Only `exclusion` removes a type from coverage consideration entirely. |
...
- **string** — CLR `string` values; `System.Type` values, canonicalized to `Type.FullName` (a `Type` with no `FullName` — an open generic parameter, a pointer, or a by-ref type — is an evidence-extraction failure); enum values, canonicalized to their **declared member name** (an enum value with no matching declared member, e.g. an undeclared combination of `[Flags]` bits or an out-of-range underlying integer, is an evidence-extraction failure — this design does not attempt to synthesize a composite flags-name string); and any literal YAML scalar that is a string.
