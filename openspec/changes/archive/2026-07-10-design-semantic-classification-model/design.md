...
3. **Same-tier conflicts — both across different mapping entries and across repeated instances of one mapped attribute — resolve by first-in-a-defined-order-wins, not by an error.** Two distinct conflict shapes exist at the `type_attribute`/`assembly_attribute` tier, and this design fixes a rule for both:
   - **Different mapping entries matching one type**: if two `classification.attributes` entries each match a different attribute present on the same type (e.g. a type carries two different marker attributes each mapped to a different role), the model deterministically picks the *first-declared mapping entry's* role/metadata — the same "YAML declaration order is the tie-break" convention already used for override entries (Decision 6) and for `ArchitectureContractFamilyBindings`'s stable enumeration order.
