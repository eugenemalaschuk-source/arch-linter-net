...
JSON gains a `when_expressions` object (`source`, `result`, `yaml_path`) on violations that have one, mirroring how `policy_location` was added additively (see `diagnostics-model`'s "Human and CI JSON formatters expose policy origin additively"). SARIF gains a related location + message referencing the same data, following the existing `relatedLocations` pattern used for policy-origin provenance. Diagnostics without a `when` omit the field entirely rather than emitting `null`, consistent with the existing "Policy location JSON has one optional-field shape" requirement.

### D3: `explain` surfaces CEL participation from already-computed violations, not a second contract run
`ArchitectureExplainApplicationService` stays a graph/shortest-path service; it does not gain general contract evaluation. Instead, the CLI-facing explain flow reuses the violations already computed during graph construction (which already carry `ExpressionParticipation` from tasks 1.2/1.3 — the same data `validate` produces), rather than re-evaluating selectors separately. This eliminates any risk of drift between `explain` and `validate` results since they are literally the same violation objects.

**Alternative considered:** re-run `ArchitectureContextSelectorMatcher` inside `explain` for each `when`-bearing selector on the resolved path. Rejected — re-evaluation can drift from `validate` if the two code paths diverge; reusing already-computed violations is simpler and guaranteed consistent.
...
- **[Risk]** Explain CEL data could drift from validate results if the two code paths diverge. → **Mitigation:** `explain` reuses the exact violation objects produced by the shared graph-build pass (which already carry `ExpressionParticipation`) rather than re-evaluating selectors separately — no separate implementation means no divergence risk.
- **[Risk]** Adding `when_expressions` to JSON/SARIF could be mistaken for a schema-version bump by downstream consumers. → **Mitigation:** field is additive and omitted when absent, consistent with existing additive-field precedent; no `schemaVersion` field changes.
