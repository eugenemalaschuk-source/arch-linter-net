...
Compatible upgrades SHALL be idempotent and maintain active, staged, and
previous verified digests from an operator-controlled shipped manifest. A
rollback SHALL target only the previous verified digest and SHALL refuse an
incompatible target without serving the older generation.
...
activate, and rollback operations through the approved admin routes. It SHALL
obtain the admin token and operator-controlled origin from process environment
variables, exact-match that origin against checked-in configuration, and never
take credential destination from repository input alone. Dry-run SHALL be
