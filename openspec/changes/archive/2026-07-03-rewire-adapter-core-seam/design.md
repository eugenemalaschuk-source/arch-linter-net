...
- **`testing-must-use-validation-application-seam` extends the same forbidden-layer set to `ArchLinterNet.Testing`**, closing a gap where CLI and Unity each had an explicit self-policy seam guardrail but Testing did not; all three first-party adapters are now symmetric.
