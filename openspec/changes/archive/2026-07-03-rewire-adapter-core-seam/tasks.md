...
- [x] 4.4 Run `make fmt`
- [x] 4.5 Run `make acceptance` (repo has no `Taskfile`/`task acceptance:fresh`; `make acceptance` is the equivalent target) and confirm it passes, including self-policy `dotnet arch-linter-net` strict/audit runs
