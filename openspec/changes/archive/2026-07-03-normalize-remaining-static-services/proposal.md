...
Issue #159 is the direct follow-up to #154/#158: the static-class inventory in `docs/internal/static-class-inventory.md` still lists 14 behavior-owning production `static class` declarations (2 with hidden `static Lazy<T>` global state) as "follow-up candidates." Until they convert, #154's architecture target — "no behavior-owning Core service remains static unless explicitly reviewed as a compatibility facade or pure helper" — is unmet, and #142's guardrail work has no enforceable baseline to police against.

## What Changes

- Convert all 14 remaining production-service static classes in section (e) of the inventory into instance classes behind focused interfaces, registered `AddSingleton` in `AddArchLinterNetCore()`:
...
- `static-production-service-inventory`: extends the "production orchestrators are instance-based" requirement to the remaining 14 classes, and adds a requirement that hidden `static Lazy<T>` singleton state is removed from production code.
...
- `src/ArchLinterNet.Core/Contracts/`, `Discovery/`, `Execution/`, `Reporting/`, `Resolution/`, `Scanning/`: 14 classes converted, 4 shadow-wrapper classes absorb the converted logic and delete their forwarded static class.
...
- `tests/ArchLinterNet.Core.Tests/**`: ~120 test call sites across the 14 classes update from static calls to instance calls (via direct `new` or DI-composed fakes, matching the existing `ArchitectureContractExecutor` test pattern).
- `docs/internal/static-class-inventory.md`: updated to move all 14 from "follow-up candidate" to "converted."
