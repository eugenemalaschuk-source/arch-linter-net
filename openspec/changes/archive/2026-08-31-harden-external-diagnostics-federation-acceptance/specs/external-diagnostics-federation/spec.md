...
the canonical finding, baseline, Human, JSON, SARIF, and Testing projections. The applicability
projection SHALL report the external-evidence control's evaluable, unassessable, or
...
- **THEN** the selected imported diagnostic appears in the canonical finding, baseline, Human,
  JSON, SARIF, and Testing projections with its evidence provenance intact
...
- **WHEN** a trusted synthetic artifact contains one diagnostic whose rule ID is authorized by the
  ...
  is not authorized
- **THEN** only the authorized diagnostic appears in selection, normalized findings, baseline,
  Human, JSON, SARIF, and Testing projections
