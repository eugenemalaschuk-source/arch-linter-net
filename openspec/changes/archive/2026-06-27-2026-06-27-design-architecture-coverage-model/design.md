...
| Term | Meaning |
|---|---|
| **covered** | The unit matches a declared layer (namespace layer, glob layer, or layer-template container expansion). A coverage contract's `roots` (or `between`/`contract_ids`) declare which units the contract classifies — they are the scan boundary, not coverage evidence themselves; being listed under `roots` does not make a unit `covered`. Only a unit that *also* matches a declared layer (or an explicit `exclude` entry) is `covered`/`excluded`; everything else under `roots` is `uncovered`. |
...
3. **Coverage scope reuses layer pattern matching; it does not introduce a new matcher.** A namespace-scope coverage contract's `roots` (the namespaces *to classify* — the scan boundary, not coverage evidence) and the existing `layers` map use the same `namespace`/`namespace_suffix` glob syntax already validated by `ArchitectureLayerResolver.MatchNamespace`. This satisfies the explicit non-goal "Unrestricted regex-based coverage matching" and the issue's instruction to prefer constrained matchers over unrestricted regex. For each concrete unit found under a contract's `roots`: it is `covered` if it matches any declared layer's pattern (including layer-template-expanded containers); `excluded` if it matches one of the coverage contract's own `exclude` entries instead; otherwise `uncovered`. Being listed under `roots` is necessary (it puts the unit in scope to be classified) but never sufficient for `covered` — a `roots` entry with no matching layer and no matching `exclude` entry is the exact `uncovered` case coverage exists to surface.
...
7. **Exclusions require `reason`; coverage severity defaults to `error`, matching `policy_consistency`.** Every `exclude` entry under a coverage contract has a mandatory `reason: string`, identical in spirit to the `reason` already required on dependency/layer/independence/protected contracts. `analysis.coverage` defaults to `error`, the same default as `policy_consistency`, for the same reason: an explicitly declared `strict_coverage` contract is intentional policy authoring, and a silent default of `off` would mean an author who wrote a coverage contract gets no signal unless they also remembered to set `analysis.coverage: error`. Backward compatibility for policies written before coverage existed does **not** depend on this default — it is guaranteed structurally (Decision 9): a policy with zero `strict_coverage`/`audit_coverage` entries has nothing for any severity setting to act on, so `analysis.coverage`'s default value is irrelevant to those policies. The default only matters once an author has written at least one coverage contract, and at that point `error` (not `off`) is the correct default because a written contract should be enforced unless the author opts out.

8. **Diagnostic identity: one new kind, one new record, mirroring `PolicyConsistencyDiagnostic`. `excluded`/`covered` are report-only and never produce a `CoverageDiagnostic`.** Add `ArchitectureDiagnosticKind.Coverage` and a `CoverageDiagnostic` record (shape only, not implemented by this change) carrying: `ContractName`, `ContractId?` (inherited from `ArchitectureDiagnostic`), `Scope` (the discriminant), `Status` (`uncovered | stale | empty-input | unknown` — exactly the subset of the vocabulary that can fail or warn validation; `covered` and `excluded` never appear here), `RepresentativeUnit` (string — namespace, project path, assembly identity, or edge description), and `Reason?` (set for `stale`/`empty-input`/`unknown` findings when the underlying contract or exclusion carries explanatory text). `excluded` units instead surface only in #102's coverage *report* as a separate aggregate (count plus, per excluded unit, its declared `reason`) — never as a `CoverageDiagnostic`, so `analysis.coverage`'s severity has nothing to act on for exclusions by construction. This follows the precedent decision in `policy-consistency-checks` design.md ("one new kind, one new record... a `CheckKind`-style discriminant... rather than a new subtype per check") — here `Scope` plus `Status` plays that discriminant role.
...
10. **The schema accepting coverage contracts requires a runtime guard, or the schema would lie about what the tool does.** The YAML loader (`ArchitectureContractLoader`) uses `IgnoreUnmatchedProperties()`, so without any C# binding, a schema-valid `strict_coverage`/`audit_coverage` contract would deserialize into nothing and be silently dropped — a policy author or AI agent could author a contract this schema says is valid, see it pass validation, and have no enforcement happen at all. That is a worse outcome than rejecting the policy outright, for a tool whose entire purpose is being a trustworthy gate. This change therefore adds a *minimal* binding (`ArchitectureCoverageContract`: `name`/`id`/`scope`/`reason` only, plus `StrictCoverage`/`AuditCoverage` lists and `Coverage` severity on the existing model types) and a single guard in `ArchitectureRunnerFactory.LoadDocument` that throws `InvalidOperationException` whenever any coverage contract is declared, regardless of `analysis.coverage`. This is not the coverage engine (no scope-specific fields are bound, no classification logic exists, no diagnostic is produced) — it is the minimum necessary to keep "schema-valid" and "enforced as authored" from silently diverging until #97 implements the real binding and engine.

## YAML Shape

```yaml
analysis:
  coverage: error   # error | warn | off — default: error (Decision 7)
...
- **[Risk] A default of `error` could fail a policy the moment its author adds a `strict_coverage` contract, before they're ready for it to gate the build.** Mitigation: this is the same trade-off already accepted for `policy_consistency`'s `error` default, and it only applies once an author has written a coverage contract — `audit_coverage` (or `analysis.coverage: warn`) is the documented escape hatch for adopting coverage without an immediate gate, mirroring how `audit` contracts let authors observe before enforcing in every other contract family.
...
5. **#103** — add architecture coverage baseline and CI gate support (revisits whether `audit_coverage`/`warn` adoption tooling needs to evolve, per Decision 7/Risk 3, once real adoption data exists).
