...
- `PolicyConsistencyDistinguisher` no longer picks `ConflictingContractIds` *or* `ConflictingContractNames` (dropping names whenever any id is present); it now builds separately-labeled `layers:...|ids:...|names:...` segments, so two findings against contracts sharing a duplicate id (a state the analyzer's own `duplicate-id` check explicitly anticipates) keep distinct identities via their still-distinct names.
...
- No production changes touched `FindDuplicateContractIds`/`FindIndependenceConflictsFor` themselves — only how their existing output is turned into an identity.
