...
Add `UsePreparedPostBuildState` to the graph and baseline-diff request models. The handler uses `EnsureBuilt` only for its first validation request, then sends ordinary, receipt-verifying requests with this flag to both graph projections and optional baseline debt. The flag directs those contributors to create a fresh isolated post-build runner before receipt verification, without restoring or building.
...
- [A prepared state is stale or absent] → the fresh post-build runner re-runs ordinary receipt verification and fails closed before graph or baseline execution.
