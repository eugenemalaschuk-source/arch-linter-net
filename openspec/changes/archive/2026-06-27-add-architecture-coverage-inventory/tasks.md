...
- [x] 3.1 Capture `document`'s declared layers (`ArchitectureLayerContract` list) in document order.
...
- [x] 4.1 Store the passed `ProjectDiscoveryResult` verbatim when present; expose as absent (nullable) when not provided.
...
- [x] 7.6 Test that validating a policy without coverage contracts does not construct the inventory (existing behavior unaffected).
