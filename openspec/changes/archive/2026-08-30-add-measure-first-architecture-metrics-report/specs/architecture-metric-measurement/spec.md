...
version and contain the same data with stable ordinal ordering; it SHALL
include the total contributor count and an explicit truncation indicator when
the requested contributor bound omits entries. The command SHALL permit an
explicit all-contributors request.
...
- **WHEN** a metric has more contributors than the selected display bound
