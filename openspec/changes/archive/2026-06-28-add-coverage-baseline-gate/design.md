...
### Decision: Treat each coverage finding's "item" as `source_type` and its discriminating reference as `forbidden_reference`
Coverage findings emitted today are `ArchitectureViolation(contract.Name, contract.Id, SourceType: <item>, ForbiddenNamespace: <category>, ForbiddenReferences: <evidence>)`, the same shape ordinary dependency violations use — and ordinary violations key their baseline entries on `(SourceType, the specific item in ForbiddenReferences)`, not on the category (`ForbiddenNamespace`). Coverage findings follow the same rule:
- Uncovered-namespace findings (`CheckCoverageContract`, namespace scope): `source_type = entry.Namespace`, `forbidden_reference = "uncovered namespace"` (a fixed marker — there is no second discriminating axis per namespace, so a stable fixed string avoids baseline churn that would come from keying on the representative type, which can shift as code changes).
- Rule-input findings (`CheckRuleInputCoverageContract`): `source_type = referencedContractId`, `forbidden_reference = layerName` — the same `layerName` value already placed in `ForbiddenReferences`, giving per-layer-reference granularity consistent with how ordinary violations key on the specific referenced type rather than a category string.

This requires no change to how violations are constructed — only that `IsIgnored` is consulted with those same two values before a finding is added to the result list.
