...
- [x] 1.2 Define project-evaluation, effective-input, expected-output, verified-artifact, completed-session, and Testing snapshot identity layers.
- [x] 1.3 Separate stable/cache identity, validation evidence, display-only evidence, and intentionally excluded machine/process fields.
...
- [x] 2.1 Define the minimum effective MSBuild/compiler/source/reference manifest included in project-evaluation fingerprints.
- [x] 2.2 Define expected output identity for project/configuration/TFM/platform/RID.
- [x] 2.3 Define authoritative verification from an ArchLinterNet receipt or equivalent supported PE/portable-PDB compiler evidence.
...
- [x] 4.4 Record that public CLI/API/diagnostic schemas and the capability manifest remain unchanged until implementation exists, to avoid advertising an unshipped capability.
- [x] 4.5 Confirm #362 already references #387 as its dedicated design dependency.

## 5. Validation and review

- [ ] 5.1 Run `rtk openspec validate --changes specify-analysis-build-state-fingerprints --strict`.
- [ ] 5.2 Run repository Markdown/docs validation required by CI.
- [ ] 5.3 Review the contract against issue #387 acceptance scenarios and the #355 Analysis/build-state slice.
- [ ] 5.4 After approval and merge, archive the change through the normal OpenSpec lifecycle before downstream implementation treats the main capability spec as published.
