...
- Whether `IArchitectureProjectDiscoveryService.ResolveAndApply` and
  `IArchitectureAssemblyResolutionService.Resolve`/`ResolvePostBuild` should take the token as a new required
  parameter or an optional one defaulting to `CancellationToken.None` — resolved during implementation by
  checking whether either interface is part of any documented/public extension surface (if internal-only,
  a required parameter is preferred for clarity per this repo's "explicit code over magic" bias; confirmed
  during implementation that both are `internal` interfaces with a single in-repo implementation each, so
  the parameter is added as required).
- Exact wiring path from `CliHost`'s new `CancellationTokenSource` to `ValidateCommandHandler`'s constructor
  — resolved during implementation by tracing `CliCompositionRoot` → `CliRootCommandFactory` →
  `CliCommandModuleCatalog` (the shallowest existing DI path), adding the token source at the same
  composition point `ICliConsole`/`IFileSystem`/`ICliRuntime` are already constructed.
