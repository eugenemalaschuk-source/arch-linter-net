...
public sealed class ArchitecturePolicyProvenanceTests
{
    ...
    public void Load_RootVersusFragmentConflict_CarriesBothTypedLocations()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(diagnostic.Location!.SourcePath, Is.EqualTo("architecture/root.yml"));
            Assert.That(diagnostic.Location.YamlPath, Is.EqualTo("layers.domain"));
            Assert.That(diagnostic.RelatedLocations.Single().SourcePath,
                Is.EqualTo("architecture/fragment.yml"));
            Assert.That(diagnostic.RelatedLocations.Single().YamlPath, Is.EqualTo("layers.domain"));
        });
    }
    ...
    public void Load_MissingSelectedRoot_CarriesRootPolicyDiagnostic()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That((string?)exception.Diagnostic.Location.Source.DeclaringSourcePath, Is.Null);
            Assert.That((string?)exception.Diagnostic.Location.Source.AuthoredImportPath, Is.Null);
