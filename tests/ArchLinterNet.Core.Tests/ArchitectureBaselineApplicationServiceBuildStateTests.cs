...
public sealed class ArchitectureBaselineApplicationServiceBuildStateTests
{
    ...
    // Tracks whether the analysis context wrapping this load scope was disposed, so tests can
    // assert that ArchitectureBaselineCandidateCollector's ResolveSetup branches dispose whichever
    // runner they materialized even when a later step (a build-state preflight) throws.
