...
public sealed partial class CheckpointBReleaseGateTests
{
    private sealed partial class CandidatePackageFeed : IDisposable
    {
        ...
        public CheckpointScenarioResult AssertPackageProvenance()
        {
            ...
            // Policy root/fragment, baseline, normalized findings, and the cache advance to 0.8.0; legacy resources stay packaged offline.
            foreach (string schema in new[]
                     {
                         "compatibility-manifest.json", "dependencies.arch.fragment.schema.json", "dependencies.arch.schema.json",
                         "baseline.schema.json", "normalized-finding.schema.json", "analysis-cache.schema.json",
                     })
            {
        ...
        // Exercises the missing-shared-framework diagnostic (see ArchitectureSharedFrameworkResolver)
        // at the public CLI surface of the installed/packed tool, not just the source-tree dll — the
        // packed nupkg/tool shim/runtime metadata boundary is exactly where issue #441 was observed.
        public CheckpointScenarioResult AssertMissingSharedFrameworkDiagnostic()
        {
            using AdoptionAcceptanceFixture fixture = AdoptionAcceptanceFixture.Create("aspnet-host");
            const string MissingFrameworkName = "Definitely.Not.An.Installed.Framework";
            File.WriteAllText(fixture.PolicyPath, File.ReadAllText(fixture.PolicyPath)
                .Replace("Microsoft.AspNetCore.App", MissingFrameworkName, StringComparison.Ordinal));
            fixture.Build();

            CommandResult result = RunTool(fixture.Root,
                "--policy", fixture.PolicyPath, "--strict", "--format", "json", "--ensure-built");

            Assert.Multiple(() =>
            {
                Assert.That(result.ExitCode, Is.EqualTo(2), result.CombinedOutput);
                Assert.That(result.CombinedOutput, Does.Contain(MissingFrameworkName));
            });
            return Passed("missing-shared-framework-diagnostic");
        }
