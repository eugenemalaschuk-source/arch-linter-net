...
public static class ArchitectureBaselineComparer
{
    ...
    private static HashSet<string> GetKnownContractIds(Families.ArchitectureContractGroups groups, string groupName)
    {
        IEnumerable<string?> ids = groupName switch
        {
            ...
            "audit_contract_surface_exposure" => groups.AuditContractSurfaceExposure.Select(c => c.Id),
            "strict_versioned_contract_surface_isolation" => groups.StrictVersionedContractSurfaceIsolation.Select(c => c.Id),
            "audit_versioned_contract_surface_isolation" => groups.AuditVersionedContractSurfaceIsolation.Select(c => c.Id),
