    ...
    : IArchitectureBaselineApplicationService
{
    public BaselineGenerationOutcome Generate(BaselineGenerationRequest request)
    {
        ...
        if (includeStrict)
        {
            ArchitectureContractExecutor.Execute(runner.Session, document, "strict", handlerRegistry, includeAsmdefContracts: false);
        }

        if (includeAudit)
        {
            ArchitectureContractExecutor.Execute(runner.Session, document, "audit", handlerRegistry, includeAsmdefContracts: false);
        }
