...
public static class ArchitectureViolationIdentityJson
{
    public static string Serialize(ArchitectureViolationIdentity identity)
    {
        ...
        return JsonSerializer.Serialize(ToWireObject(identity));
    }

    /// <summary>Returns the stable, snake-case wire fields used by every normalized adapter.</summary>
    public static IReadOnlyDictionary<string, object?> ToWireObject(ArchitectureViolationIdentity identity)
    {
        ArgumentNullException.ThrowIfNull(identity);
