...
// Canonical finding-identity attribution, extracted from ArchitectureAnalysisSession (issue #452).
//
// It is a pure function of (candidate log, cursor, violations): it reads the session's
// finding-identity candidate log, never writes to it, and holds no state between calls, so identity
// attribution can be exercised without driving a mutable session lifecycle. The session still owns
// the candidate log itself, and ArchitectureContractExecutor still calls this exactly once per
// contract, after that contract's checking completed — so a cancelled run either has a contract's
// fully attributed findings or none of them, never a half-attributed set.
internal static class ArchitectureFindingIdentityAttributor
{
    public static IReadOnlyList<ArchitectureViolation> Attach(
        IReadOnlyList<ArchitectureBaselineCandidate> candidateLog,
        int cursor,
        IReadOnlyCollection<ArchitectureViolation> violations)
    {
        ...
        Dictionary<CandidateBucketKey, List<ArchitectureBaselineCandidate>> available =
            BuildCandidateBuckets(candidateLog, cursor);
    ...
    private static Dictionary<CandidateBucketKey, List<ArchitectureBaselineCandidate>> BuildCandidateBuckets(
        IReadOnlyList<ArchitectureBaselineCandidate> candidateLog, int cursor)
    {
        Dictionary<CandidateBucketKey, List<ArchitectureBaselineCandidate>> buckets = new();

        for (int i = cursor; i < candidateLog.Count; i++)
        {
            ArchitectureBaselineCandidate candidate = candidateLog[i];
