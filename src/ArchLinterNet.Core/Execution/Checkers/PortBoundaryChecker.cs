...
internal static class PortBoundaryChecker
{
    ...
    public static List<ArchitectureViolation> Check(
        ArchitecturePortBoundaryContract contract,
        ArchitectureCheckerContext checkerContext,
        ArchitectureContractExecutionContext context)
    {
        List<ArchitectureViolation> violations = new();
        foreach (Type source in checkerContext.FindContextSelectorMatchingTypes(contract.Source))
        {
            if (!checkerContext.RoleIndex.TryGetRole(source, out ArchitectureTypeClassificationResult sourceRole)) continue;
            ...
            foreach (Type target in referencedTypes.Distinct())
            {
                ArchitectureViolation? violation = TryBuildDirectEdgeViolation(
                    contract, checkerContext, context, sourceRole, sourceName, sourceAssembly, target);
                if (violation != null) violations.Add(violation);
            }
        }
        CollectAdapterBindingViolations(contract, checkerContext, context, violations);
        return violations;
    }

    private static ArchitectureViolation? TryBuildDirectEdgeViolation(ArchitecturePortBoundaryContract contract,
        ArchitectureCheckerContext checkerContext, ArchitectureContractExecutionContext context,
        ArchitectureTypeClassificationResult sourceRole, string sourceName, string sourceAssembly, Type target)
    {
        if (!MatchesTargetContext(contract.TargetContext, checkerContext, target, sourceRole)
            || checkerContext.IsExcludedFromContextMatch(target, contract.Exclude, sourceRole)) return null;
        bool matchesForbidden = contract.Forbidden.Any(s =>
            ArchitectureContextSelectorMatcher.Matches(s, target, checkerContext.RoleIndex, sourceRole));
        bool matchesAllowedSeam = contract.AllowedSeams.Any(s =>
            ArchitectureContextSelectorMatcher.Matches(s, target, checkerContext.RoleIndex, sourceRole));
        ...
        checkerContext.RoleIndex.TryGetRole(target, out ArchitectureTypeClassificationResult targetRole);
        string expectedSeam = string.Join(" or ", contract.AllowedSeams.Select(ContextualCheckerSupport.DescribeSelector));
    ...
    private static void CollectAdapterBindingViolations(ArchitecturePortBoundaryContract contract,
        ArchitectureCheckerContext checkerContext, ArchitectureContractExecutionContext context,
        List<ArchitectureViolation> violations)
    {
        foreach (ArchitectureAdapterPortBinding binding in contract.AdapterBindings)
            foreach (Type adapter in checkerContext.FindContextSelectorMatchingTypes(binding.Adapter))
            {
                ArchitectureViolation? violation =
                    TryBuildAdapterBindingViolation(contract, binding, checkerContext, context, adapter);
                if (violation != null) violations.Add(violation);
            }
    }

    private static ArchitectureViolation? TryBuildAdapterBindingViolation(ArchitecturePortBoundaryContract contract,
        ArchitectureAdapterPortBinding binding, ArchitectureCheckerContext checkerContext,
        ArchitectureContractExecutionContext context, Type adapter)
    {
        if (!checkerContext.RoleIndex.TryGetRole(adapter, out ArchitectureTypeClassificationResult adapterRole)) return null;
        bool inAllowedContext = binding.AllowedContexts.Count == 0 || binding.AllowedContexts.Any(selector =>
            ArchitectureContextSelectorMatcher.Matches(selector, adapter, checkerContext.RoleIndex, adapterRole));
        ...
        Type? implementedExpectedPort = implementedInterfaces
            ...
            .FirstOrDefault(@interface => ArchitectureContextSelectorMatcher.Matches(
                binding.ExpectedPort, @interface, checkerContext.RoleIndex, adapterRole));
        ...
        Type? actualPort = implementedExpectedPort ?? implementedInterfaces
            .OrderByDescending(@interface =>
                checkerContext.RoleIndex.TryGetRole(@interface, out ArchitectureTypeClassificationResult r) && r.Role != null)
            .ThenBy(ArchitectureTypeNames.SafeFullName, StringComparer.Ordinal)
            .FirstOrDefault();
        ArchitectureTypeClassificationResult? actualPortRole = null;
        if (actualPort != null
            && checkerContext.RoleIndex.TryGetRole(actualPort, out ArchitectureTypeClassificationResult resolvedPortRole))
        {
        ...
        string detail = implementsExpectedPort
            ...
            : $"adapter implements {actualPortName}, not expected port {ContextualCheckerSupport.DescribeSelector(binding.ExpectedPort)}";
        ...
        return new ArchitectureViolation(contract.Name, contract.Id, adapterName, detail, new[] { actualPortName })
        {
            Payload = new PortBoundaryPayload(
                ...
                kind,
                ContextualCheckerSupport.DescribeSelector(binding.ExpectedPort),
    ...
    private static bool MatchesTargetContext(ArchitectureContextMetadataSelector selector,
        ArchitectureCheckerContext checkerContext, Type target, ArchitectureTypeClassificationResult source)
    {
        if (!checkerContext.RoleIndex.TryGetRole(target, out ArchitectureTypeClassificationResult targetRole)) return false;
